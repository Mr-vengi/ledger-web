...
/// ✅ Export to PDF with web support
...
_buildPDFCell('Description', bold: true, align: pw.TextAlign.center),
...
_buildPDFCell('Closing (Rs)', bold: true, align: pw.TextAlign.center),
],
),
...
maxLines: 2,
bold: isGrandTotal || isSalesValue || isOpeningBalance,
),
_buildPDFCell(
tx['description'],
align: pw.TextAlign.left,
...
await _saveOrDownloadFile(pdfBytes, shopName, 'pdf');
}

/// ✅ Export to Excel with web support
...
_styleExcelCell(sheet, row, 0, isCenter: true, isBold: isGrandTotal || isSalesValue);
_styleExcelCell(sheet, row, 1, isLeft: true, isBold: isGrandTotal || isSalesValue || isOpeningBalance);
_styleExcelCell(sheet, row, 2, isLeft: true, isBold: isGrandTotal || isSalesValue);
_styleExcelCell(sheet, row, 3, isCenter: true, isBold: isGrandTotal || isSalesValue);
_styleExcelCell(sheet, row, 4, isRight: true, isBold: isGrandTotal || isSalesValue);
_styleExcelCell(sheet, row, 5, isRight: true, isBold: isGrandTotal || isSalesValue);
_styleExcelCell(sheet, row, 6, isRight: true, isBold: isGrandTotal || isOpeningBalance);
...
if (excelBytes != null) {
await _saveOrDownloadFile(Uint8List.fromList(excelBytes), shopName, 'xlsx');
}
}

/// ✅ Unified save/download method for web and mobile
Future<void> _saveOrDownloadFile(Uint8List bytes, String shopName, String format) async {
final fileName = 'ledger_${shopName.toLowerCase().replaceAll(' ', '_')}_${DateFormat('dd-MMM-yyyy').format(_startDate!)}_${DateTime.now().millisecondsSinceEpoch}.$format';

if (_isWeb) {
// ✅ WEB: Download using html
final blob = html.Blob([bytes], format == 'pdf'
? 'application/pdf'
...
debugPrint('✅ $format saved to: $filePath');
...
excel_pkg.CellIndex.indexByColumnRow(columnIndex: col, rowIndex: row),
    ...
    .value = excel_pkg.TextCellValue(text);
}
