...
/// ✅ NEW: Save/Share Excel (Works on Web + Mobile)
static Future<void> saveOrShareExcel({
...
await _downloadExcelOnWeb(Uint8List.fromList(excelBytes), fileName);
_showSuccessMessage(context, 'Excel downloaded successfully!');
} else {
// ✅ MOBILE: Save and share
await _saveAndShareExcelOnMobile(Uint8List.fromList(excelBytes), fileName, context);
}
} catch (e) {
debugPrint('Error saving/sharing Excel: $e');
_showErrorMessage(context, 'Failed to save Excel: ${e.toString()}');
}
}

/// ✅ Web download helper
static Future<void> _downloadExcelOnWeb(Uint8List excelBytes, String fileName) async {
try {
// Only execute on web
if (!_isWeb) return;
...
debugPrint('✅ Excel downloaded on web: $fileName');
} catch (e) {
...
static Future<void> _saveAndShareExcelOnMobile(
...
debugPrint('✅ Excel saved temporarily: $filePath');
...
await file.delete();
debugPrint('✅ Temporary Excel deleted');
}
} catch (e) {
